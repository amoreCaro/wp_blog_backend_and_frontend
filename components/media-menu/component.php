...
<div class="media-menu absolute w-full top-[80px] z-50 bg-[#F6F5F8] flex flex-col py-2">
    <div class="container w-full mx-auto px-5 xl:px-10 2xl:px-0">
        ...
        <div class="media-menu__head flex items-center justify-between">
            <!-- Category Button для мобільного -->
            ...
            <div class="media-menu__categories-content hidden md:block overflow-x-auto no-scrollbar">
                <ul class="flex items-center gap-8 whitespace-nowrap">
                    <?php $all_news_active = ( is_page('blog') ) ? 'active' : ''; ?>
                    <li>
                        <a href="<?php echo esc_url(home_url('/blog/')); ?>" class="media-menu__tab <?php echo esc_attr($all_news_active); ?> uppercase block py-3 text-[15px] leading-[18px] font-bold hover:text-black">
                            All News
                        </a>
                    </li>
                    ...
                    ?>
                    <li>
                        <a href="<?php echo esc_url(get_category_link($category->term_id)); ?>" class="media-menu__tab <?php echo esc_attr($is_active); ?> uppercase block py-3 text-[15px] leading-[18px] font-bold hover:text-black">
        ...
        <div class="media-menu__navigation transition-all duration-500 max-h-0 opacity-0 mobile">
            <ul class="flex flex-col sm:flex-row items-start sm:items-center justify-start sm:justify-between w-full whitespace-nowrap">
                ...
                <li class="flex-1 text-left">
                    <a href="<?php echo esc_url(home_url('/blog/')); ?>" class="media-menu__tab <?php echo esc_attr($all_news_mobile_active); ?> uppercase py-3 text-[15px] leading-[18px] font-bold inline-block relative hover:text-black">
                        All News
                    </a>
                </li>
                ...
                ?>
                <li class="flex-1 text-left">
                    <a href="<?php echo esc_url(get_category_link($category->term_id)); ?>" class="media-menu__tab <?php echo esc_attr($is_active_mobile); ?> uppercase py-3 text-[15px] leading-[18px] font-bold inline-block relative hover:text-black">
                        <?php echo esc_html($category->name); ?>
                    </a>
                </li>
                <?php endforeach; ?>
            ...
            <ul class="media-menu__tags-list grid grid-cols-1 md:grid-cols-3 gap-y-4 py-8">
                <?php if (!empty($tags)) : ?>
                    <?php foreach ($tags as $tag) : ?>
                        <li>
                            <a href="<?php echo esc_url(get_tag_link($tag->term_id)); ?>" class="media-menu__tag uppercase text-[15px] leading-[18px] font-bold text-[#9395ab] hover:text-[#252735] transition-all duration-300 cursor-pointer">
